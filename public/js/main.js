...
(function () {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (!card) return;
    const href = card.dataset.href || card.querySelector('a[href]')?.href;
  ...
  function runHeroEntrance() {
    ...
    gsap.fromTo('.nav-logo',
      { opacity: 0, y: -12 },
      { opacity: 1, y: 0, duration: 0.8, ease: EASE.elegant, delay: 0.3 }
    );

    gsap.fromTo('.nav-links li',
      { opacity: 0, y: -8 },
      { opacity: 1, y: 0, stagger: 0.05, duration: 0.6, ease: EASE.elegant, delay: 0.5 }
    );

    gsap.fromTo('.nav-actions',
      { opacity: 0, y: -8 },
      { opacity: 1, y: 0, duration: 0.6, ease: EASE.elegant, delay: 0.6 }
    );
  ...
  if (window.matchMedia('(pointer: fine)').matches) {
    document.querySelectorAll('.card').forEach(card => {
      gsap.set(card, { rotateX: 0, rotateY: 0, y: 0, transformPerspective: 800 });
